...
public class MondayFragment extends Fragment implements OnClickListener
{
	...
	public void onResume()
	{
		...
		try 
		{
			na = new NewNodeXmlParser(getActivity().getAssets().open("muhlenberg_dining.xml"), Calendar.MONDAY);
			na.parse();
		} 
		...
		//initialize
		meals[0] = (TextView) getActivity().findViewById(R.id.mondayBreakfast);
		meals[1] = (TextView) getActivity().findViewById(R.id.mondayLunch);
		meals[2] = (TextView) getActivity().findViewById(R.id.mondayDinner);

		mealContents[0] = (TextView) getActivity().findViewById(R.id.mondayBreakfastContent);
		mealContents[1] = (TextView) getActivity().findViewById(R.id.mondayLunchContent);
		mealContents[2] = (TextView) getActivity().findViewById(R.id.mondayDinnerContent);
		...
		for(int i=0;i<titles.length;i++)
		{
			...
			meals[i].setText(Html.fromHtml("<b>" + titles[i] + "</b>"));
	...
	{
		int index = -1;
		if(v.getId() == R.id.mondayBreakfast)
		{
			index = 0;
